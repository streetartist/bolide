# Benchmark runner — external timing, Go / C / Swift / Python / Bolide JIT / Bolide AOT
param([int]$Runs = 5)
...
$benchmarks = @(
    @{Name="fib_rec(40)"; C="$Root\bench\fib_rec_c.exe"; Go="$Root\bench\fib_rec_go.exe"; Swift="$Root\bench\fib_rec_swift.exe"; BL="$Root\bench\fib_rec.bl"; BLAot="$Root\bench\fib_rec_bl.exe"; Py="$Root\bench\fib_rec.py"},
    @{Name="sieve(50M)";  C="$Root\bench\sieve_c.exe";   Go="$Root\bench\sieve_go.exe";   Swift="$Root\bench\sieve_swift.exe";   BL="$Root\bench\sieve.bl";   BLAot="$Root\bench\sieve_bl.exe";   Py="$Root\bench\sieve.py"}
)
...
foreach ($bm in $benchmarks) {
    ...
    $swift = Invoke-Timed "Swift 6.3 (-O)"  { & $bm.Swift }           $Runs
    ...
    Write-Host "  " + ("-" * 48)
    foreach ($r in @($c, $go, $swift, $aot, $jit, $py) | Sort-Object Avg) {
