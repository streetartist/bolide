...
use bolide_parser::{
    ...
    Program, Statement, Type as BolideType, UnaryOp, ValueField, VarDecl,
};
...
pub struct AotCompiler {
    ...
    /// FuncSig raw→closure adapter definitions
...
impl AotCompiler {
    ...
    fn collect_strings_from_expr(&self, expr: &Expr, strings: &mut HashSet<String>) {
        match expr {
            ...
            Expr::NamedArg(name, value) => {
                strings.insert(format!("{}\0", name));
                self.collect_strings_from_expr(value, strings);
            }
            Expr::SpreadArg(value) | Expr::KwSpreadArg(value) => {
                self.collect_strings_from_expr(value, strings);
            }
            Expr::BinOp(l, _, r) => {
    ...
    pub fn compile(mut self, program: &Program) -> Result<AotCompileResult, String> {
        ...
        // 宏展开（`name!` / `@derive` / `@test`）
        let program =
            crate::expand_macros(program).map_err(|e| format!("Macro error: {}", e))?;
        // trait / mixin 脱糖
        let program =
        ...
        self.funcsig_return_sources = self.collect_funcsig_return_sources(&program);
        self.funcsig_closure_param_indices = self.collect_funcsig_closure_param_indices(&program);
        // Collect global types early so list<func(...)> etc. get adapters.
        self.precollect_global_var_types(&program);
        ...
        for stmt in &program.statements {
            if let Statement::VarDecl(decl) = stmt {
                ...
                let data_size = self.type_storage_size(&gvar_ty);
                self.data_desc.define_zeroinit(data_size as usize);
                self.module
                    .define_data(data_id, &self.data_desc)
    ...
    fn infer_expr_type_static(&self, expr: &Expr) -> Option<BolideType> {
        match expr {
            ...
            Expr::ValueConstruct(type_name, _) => Some(BolideType::Custom(type_name.clone())),
            Expr::List(items) => {
                        ...
                        class_name.and_then(|name| self.lookup_method_return_type(&name, method))
                    {
                        ...
                        match self.infer_expr_type_static(base) {
                            ...
                            Some(BolideType::Str) => match method.as_str() {
                                ...
                                | "format" | "substring" | "char_at" => Some(BolideType::Str),
    ...
    ///
    /// 标准库短路径：`std/fs` / `std/fs.bl` → `std/fs/fs.bl`
    ...
    fn declare_function(&mut self, func: &FuncDef) -> Result<(), String> {
        ...
        for param in &func.params {
            ...
            let ty = if param.mode == ParamMode::Ref {
                self.ptr_type
            } else {
                self.bolide_type_to_cranelift(&param_ty)
            };
    ...
    fn value_type_cranelift_types(&self, name: &str) -> Vec<types::Type> {
        match self.value_types.get(name) {
            Some(fields) => fields
                .iter()
                .map(|field| self.bolide_type_to_cranelift(&field.ty))
                .collect(),
            None => vec![],
        }
    }

    fn type_storage_size(&self, ty: &BolideType) -> u64 {
        match ty {
            BolideType::Custom(name) if self.value_types.contains_key(name) => {
                (self.value_types.get(name).map(|fields| fields.len()).unwrap_or(0) * 8) as u64
            }
            _ => 8,
        }
    }
            ...
            if let Statement::ClassDef(class) = stmt {
                ...
                for method in &class.methods {
                    ...
                    let method_name = if is_overloaded {
                        method_overload_name(&class.name, &method.name, &method.params)
                    } else {
                ...
                Statement::ClassDef(class_def) => {
                    ...
                    for method in &class_def.methods {
                        if matches!(
                            ...
                            let method_full_name = if is_overloaded {
                            ...
                            funcs.push((method_full_name, params, method.body.clone()));
                        }
                    }
                }
                _ => {}
    ...
    fn precollect_global_var_types(&mut self, program: &Program) {
        for stmt in &program.statements {
            if let Statement::VarDecl(decl) = stmt {
                if let Some(ty) = &decl.ty {
                    self.global_var_types
                        .insert(decl.name.clone(), ty.clone());
                }
            }
        }
    }
    ...
    fn compile_funcsig_raw_adapters(&mut self) -> Result<(), String> {
        let sigs = if self.funcsig_adapter_defs.is_empty() {
            self.funcsig_raw_adapter_sigs()
        } else {
            self.funcsig_adapter_defs.clone()
        };
        for (name, (params, ret)) in sigs {
    ...
    fn compile_class_constructor(&mut self, class_name: &str) -> Result<(), String> {
        ...
        for field in &class_info.fields {
            ...
            if let Some(clone_name) = AotCompileContext::get_clone_func_name(&field.ty) {
        ...
        for (i, field) in class_info.fields.iter().enumerate() {
            let mut param = builder.block_params(entry)[i];
            ...
            if !matches!(field.ty, BolideType::FuncSig(_, _) | BolideType::Func) {
                if let Some(clone_name) = AotCompileContext::get_clone_func_name(&field.ty) {
                    if let Some(&clone_ref) = clone_refs.get(clone_name) {
                        let call = builder.ins().call(clone_ref, &[param]);
                        param = builder.inst_results(call)[0];
                    }
                }
            }
        ...
        for stmt in &program.statements {
            if let Statement::ClassDef(class) = stmt {
                ...
                for method in &class.methods {
                    ...
                    self.compile_class_method(&class.name, &method_with_self)?;
                }
            }
        }
        Ok(())
    ...
    fn compile_class_method(&mut self, class_name: &str, method: &FuncDef) -> Result<(), String> {
        ...
        // 使用作用域来确保 ctx 在 finalize 之前被释放
        {
            let mut ctx = AotCompileContext::new(
                ...
                self.global_var_mutability.clone(),
    ...
    fn compile_function_internal(
        ...
        for param in &func.params {
            ...
            let sig_ty = if param.mode == ParamMode::Ref {
                self.ptr_type
            } else {
                self.bolide_type_to_cranelift(&param_ty)
            };
        ...
        // 使用作用域来确保 ctx 在 finalize 之前被释放
        {
            ...
            for (i, param) in func.params.iter().enumerate() {
                ...
                if let BolideType::Custom(vt_name) = &param_ty {
                    if let Some(fields) = ctx.value_types.get(vt_name).cloned() {
                        for (fi, field) in fields.iter().enumerate() {
                            let ft = ctx.bolide_type_to_cranelift(&field.ty);
                            let fvar_name = format!("{}${}", param.name, fi);
                            let var = ctx.declare_variable(&fvar_name, ft);
                            ctx.builder.def_var(var, params[flat]);
                            flat += 1;
                        }
                        ctx.value_var_fields.insert(param.name.clone(), fields);
                        continue;
                    }
                }
            ...
            if !returned {
                ...
                if func.return_type.is_some() {
                    if has_sret {
                        let zero_i64 = ctx.builder.ins().iconst(types::I64, 0);
                        let zero_f64 = ctx.builder.ins().f64const(0.0);
                        if let Some(sret_ptr) = ctx.sret_ptr {
                            if let Some(BolideType::Custom(vt_name)) = func
                                .return_type
                                .as_ref()
                                .map(|ty| ctx.normalize_bolide_type(ty))
                            {
                                if let Some(fields) = ctx.value_types.get(&vt_name).cloned() {
                                    for (fi, field) in fields.iter().enumerate() {
                                        let z = if field.ty == BolideType::Float {
                                            zero_f64
                                        } else {
                                            zero_i64
                                        };
    ...
    fn compile_closure_job(
...

        {
            ...
            if let Some(ref ret) = job.return_type {
                ctx.func_return_types
                    .insert(job.name.clone(), Some(ret.clone()));
            } else {
                ctx.func_return_types.insert(job.name.clone(), None);
            }
            ...
            for (i, (name, ty)) in job.captures.iter().enumerate() {
                ...
                if matches!(ty, BolideType::FuncSig(_, _) | BolideType::Func) {
                    ctx.closure_param_vars.insert(name.clone());
                }
            }
...
impl<'a, 'b> AotCompileContext<'a, 'b> {
    ...
    fn current_module_name(&self) -> Option<&str> {
        if !self.current_func.starts_with('@') {
            return None;
        }
        self.modules
            .keys()
            .filter(|module| {
                let prefix = format!("@{}_", module);
                self.current_func.starts_with(&prefix)
            })
    ...
    fn emit_default_return_for_exception(&mut self) {
        let return_ty = self
            ...
            .flatten();
        match return_ty {
            Some(BolideType::Custom(name)) if self.value_types.contains_key(&name) => {
                let zero_i64 = self.builder.ins().iconst(types::I64, 0);
                let zero_f64 = self.builder.ins().f64const(0.0);
                if let (Some(sret_ptr), Some(fields)) = (
                    self.sret_ptr,
                    self.value_types.get(&name).cloned(),
                ) {
                    for (fi, field) in fields.iter().enumerate() {
                        let zero = if field.ty == BolideType::Float {
                            zero_f64
                        } else {
                            zero_i64
                        };
    ...
    fn value_type_name_of(&self, expr: &Expr) -> Option<String> {
        match self
            ...
            .map(|ty| self.normalize_bolide_type(&ty))
        {
            Some(BolideType::Custom(name)) if self.value_types.contains_key(&name) => Some(name),
            _ => None,
        }
    }
        ...
        match expr {
            Expr::ValueConstruct(type_name, fields) => {
                ...
                for field in &def_fields {
                    if let Some((_, fexpr)) = fields.iter().find(|(name, _)| name == &field.name) {
            ...
            Expr::Ident(name) => {
                ...
                let gv = *self
                    .global_refs
                    .get(&self.resolved_global_name(name))
            ...
            Expr::Member(base, member) => {
                ...
                let vt_name = self
                    ...
                    .ok_or_else(|| "member access on non-value-type".to_string())?;
                let fields = self
                    .value_types
                    .get(&vt_name)
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<Value, String> {
        match expr {
            ...
            Expr::ValueConstruct(type_name, _) => Err(format!(
                "value type '{}' cannot be used as a single scalar here; assign it to a variable, pass it to a function, or return it",
                type_name
            )),
    ...
    fn compile_ident(&mut self, name: &str) -> Result<Value, String> {
        ...
        let resolved_global = self.resolved_global_name(name);
        if let Some(&gv) = self.global_refs.get(&resolved_global) {
            let addr = self.builder.ins().global_value(self.ptr_type, gv);
            let load_ty = self
                .global_var_types
                .get(&resolved_global)
    ...
    fn compile_binop(&mut self, left: &Expr, op: &BinOp, right: &Expr) -> Result<Value, String> {
        ...
        // 类类型运算符重载（左优先，再右反射）
        if let Some(result) =
            self.try_binop_operator_overload(left, op, right, &left_type, &right_type)?
        {
            return Ok(result);
        }

        if matches!(op, BinOp::Add) {
            if let (Some(BolideType::List(left_elem)), Some(BolideType::List(right_elem))) =
    ...
    fn try_binop_operator_overload(
        ...
        if let Some(BolideType::Custom(class_name)) = left_type {
            if let Some(method) = crate::operator_overload::binop_method(op) {
                if self.find_class_method(class_name, method).is_some() {
                    let result = self.compile_method_call(left, method, &[right.clone()])?;
                    return Ok(Some(result));
                }
            }
        }
        if let Some(BolideType::Custom(class_name)) = right_type {
    ...
    fn infer_binop_overload_type(
        &self,
        left_ty: &Option<BolideType>,
        op: &BinOp,
        right_ty: &Option<BolideType>,
    ) -> Option<BolideType> {
        if let Some(BolideType::Custom(cls)) = left_ty {
            if let Some(method) = crate::operator_overload::binop_method(op) {
        ...
        if let Some(BolideType::Custom(cls)) = right_ty {
            if let Some(method) = crate::operator_overload::reflected_binop_method(op) {
                if let Some(ret) = self.lookup_method_return_type(cls, method) {
                    return Some(ret);
                }
    ...
    fn compile_format_name_string(&mut self, name: &str) -> Result<Value, String> {
        let c_name = self.create_c_string_constant(name)?;
        let func_ref = *self
            .func_refs
            .get("@_string_new")
            .ok_or("string_new not found")?;
        let call = self.builder.ins().call(func_ref, &[c_name]);
        let result = self.builder.inst_results(call)[0];
        self.track_temp_rc_value(result, &BolideType::Str);
        Ok(result)
    }
    ...
    fn compile_string_format(&mut self, template: Value, args: &[Expr]) -> Result<Value, String> {
        ...
        for arg in args {
            match arg {
                Expr::NamedArg(name, value) => {
                    let name_value = self.compile_format_name_string(name)?;
                    let formatted_value =
                        self.compile_to_str(std::slice::from_ref(value.as_ref()))?;
                ...
                expr => {
                    positional.push(self.compile_to_str(std::slice::from_ref(expr))?);
                }
            }
        }
    ...
    fn compile_unary(&mut self, op: &UnaryOp, operand: &Expr) -> Result<Value, String> {
        ...
        if let Some(BolideType::Custom(ref class_name)) = operand_type {
    ...
    fn compile_call(&mut self, callee: &Expr, args: &[Expr]) -> Result<Value, String> {
        match callee {
            ...
            other => {
                ...
                self.compile_indirect_call_ptr(func_ptr, args, None)
            }
        }
    }
    ...
    fn compile_method_call(
        ...
        if let Some(ref ty) = base_type {
            if Self::is_mutating_method(ty, method_name) {
        ...
        if let Some(BolideType::Custom(class_name)) = base_type {
            ...
            let base_val = self.compile_expr(base)?;
            let arg_types: Option<Vec<BolideType>> =
    ...
    fn compile_list_method(
        ...
        match method_name {
            ...
            "set" => {
                ...
                let call = self.builder.ins().call(f, &[list_val, idx, value]);
                Ok(self.builder.inst_results(call)[0])
            }
            "insert" => {
                ...
                let value = self.compile_expr(&args[1])?;
                let value = self.prepare_list_runtime_value(value, &args[1], &list_elem_ty)?;
                self.builder.ins().call(f, &[list_val, idx, value]);
                Ok(self.builder.ins().iconst(types::I64, 0))
            }
            "remove" => {
                let f = *self
    ...
    fn compile_string_method(
        ...
        match method_name {
            ...
            "format" => self.compile_string_format(str_val, args),
            "substring" | "substr" => {
    ...
    fn compile_named_call(&mut self, name: &str, args: &[Expr]) -> Result<Value, String> {
        ...
        let is_func_var = (self.variables.contains_key(name)
            || self.global_refs.contains_key(&resolved_global))
            && matches!(
                var_func_ty,
                Some(BolideType::Func) | Some(BolideType::FuncSig(_, _))
            );
        ...
        let params = self
            ...
            .cloned()
            .unwrap_or_default();
        let mut arg_vals: Vec<Option<Vec<Value>>> = vec![None; params.len()];
        for (i, param) in params.iter().enumerate() {
            if param.is_variadic {
                arg_vals[i] = Some(vec![self.new_packed_args(&param.ty)?]);
            } else if param.is_kw_variadic {
                arg_vals[i] = Some(vec![self.new_packed_kwargs(&param.ty)?]);
            }
        }
                ...
                ParamMode::Borrow => match arg {
                    PreparedArg::Expr { expr, .. } => {
                        if self.value_type_name_of(expr).is_some() {
                            arg_vals[target_index] = Some(self.compile_value_expr(expr)?);
                            continue;
                        }
                        let val = self.compile_expr(expr)?;
                        ...
                        arg_vals[target_index] = Some(vec![val]);
                    }
                    PreparedArg::PackedArgItem { elem_ty, item, .. } => {
                        let list_ptr = arg_vals[target_index]
                            .as_ref()
                    ...
                    PreparedArg::PackedKwargItem { value_ty, item, .. } => {
                        let dict_ptr = arg_vals[target_index]
                            .as_ref()
                            .and_then(|vals| vals.first().copied())
                            .ok_or_else(|| {
                ...
                ParamMode::Owned => {
                    ...
                    if let Expr::Ident(var_name) = expr {
                        ...
                        if let Some(&var) = self.variables.get(var_name) {
                            ...
                            self.global_refs.get(&self.resolved_global_name(var_name))
                        {
                            let addr = self.builder.ins().global_value(self.ptr_type, gv);
                            let null = self.builder.ins().iconst(self.ptr_type, 0);
                ...
                ParamMode::Ref => {
                    ...
                    if let Expr::Ident(var_name) = expr {
                        if let Some(&var) = self.variables.get(var_name) {
                            ...
                            arg_vals[target_index] = Some(vec![slot_addr]);
                            ...
                            self.global_refs.get(&self.resolved_global_name(var_name))
                        {
                            ...
                            let addr = self.builder.ins().global_value(self.ptr_type, gv);
                            arg_vals[target_index] = Some(vec![addr]);
        ...
        let ret_is_vt = self
            ...
            .cloned()
            .flatten()
            .and_then(|rt| match self.normalize_bolide_type(&rt) {
                BolideType::Custom(name) if self.value_types.contains_key(&name) => Some(name),
                _ => None,
            });
        let sret_slot = if let Some(vt_name) = ret_is_vt.as_ref() {
            let fields = self.value_types.get(vt_name).cloned().unwrap_or_default();
            ...
            Some(self.builder.ins().stack_addr(self.ptr_type, slot, 0))
        } else {
            None
        };

        let mut flat_args: Vec<Value> = if let Some(ptr) = sret_slot { vec![ptr] } else { vec![] };
        for value in arg_vals.into_iter() {
            let vals = value.ok_or_else(|| "internal error: missing prepared argument".to_string())?;
            flat_args.extend(vals);
        }

        // 调用函数
        let call = self.builder.ins().call(func_ref, &flat_args);
        self.emit_exception_pending_check()?;
        ...
        for (var_name, slot_addr) in &ref_slots {
            if let Some(&var) = self.variables.get(var_name) {
                ...
                .global_refs
                .contains_key(&self.resolved_global_name(var_name))
            {
        ...
        if results.is_empty() {
            ...
            let ret_ty_opt = self.func_return_types.get(&func_name).cloned().flatten();
                ...
                if matches!(ret_ty, BolideType::FuncSig(_, _) | BolideType::Func) {
                    ...
                    if self.direct_call_returns_raw_funcsig(&func_name, args) {
                        if let BolideType::FuncSig(params, ret) = &ret_ty {
                            result = self.wrap_raw_funcsig_as_closure(result, params, ret)?;
                        }
                    }
    ...
    fn compile_index(&mut self, base: &Expr, index: &Expr) -> Result<Value, String> {
        ...
        match base_type {
            ...
            Some(BolideType::Tuple(ref inner_types)) => {
                let func_ref = *self
                    .func_refs
                    .get("@_tuple_get")
                    .ok_or("tuple_get not found")?;
                ...
                // tuple_get returns i64 for all types; bitcast to F64 for float elements
                if let Expr::Int(idx) = index {
                    let idx = *idx as usize;
                    if idx < inner_types.len() && matches!(inner_types[idx], BolideType::Float) {
        ...
        match expr {
            Expr::Ident(name) => {
                ...
                if self.func_refs.contains_key(&func_lookup) {
                    ...
                    let ret = self
                        ...
                        .flatten()
                        .map(Box::new);
                    return Some(BolideType::FuncSig(params, ret));
                }
                None
            }
            Expr::Int(_) => Some(BolideType::Int),
            ...
            Expr::BinOp(left, op, right) => {
                ...
                match (&left_ty, &right_ty) {
                    (Some(BolideType::List(left_elem)), Some(BolideType::List(right_elem))) => {
                        match op {
                            BinOp::Add if left_elem == right_elem => {
                                Some(BolideType::List(left_elem.clone()))
                            }
            ...
            Expr::UnaryOp(op, operand) => {
                ...
                if let Some(BolideType::Custom(ref cls)) = operand_ty {
                    if let Some(method) = crate::operator_overload::unary_method(op) {
                        if let Some(ret) = self.lookup_method_return_type(cls, method) {
                            return Some(ret);
                        }
            ...
            Expr::Member(base, member) => {
                ...
                if let Some(class_name) = class_name {
                    ...
                    if let Some(fields) = self.value_types.get(&class_name) {
    ...
    fn compile_list_with_hint(
        ...
        for item in items {
            ...
            if elem_ty == BolideType::Dynamic {
                let actual_ty = self
                    .normalize_bolide_type(&self.infer_expr_type(item).unwrap_or(BolideType::Int));
                if actual_ty != BolideType::Dynamic {
                    val = self.convert_to_dynamic(val, &actual_ty)?;
                }
            }
    ...
    fn prepare_list_runtime_value(
        ...
        if *list_elem_ty == BolideType::Dynamic {
            let actual_ty = self
                .normalize_bolide_type(&self.infer_expr_type(expr).unwrap_or(BolideType::Int));
            if actual_ty != BolideType::Dynamic {
    ...
    fn compile_spawn(
        ...
        self.check_spawn_args_thread_safe(name, args)?;
    ...
    fn check_spawn_args_thread_safe(&self, func_name: &str, args: &[Expr]) -> Result<(), String> {
        for (index, arg) in args.iter().enumerate() {
            if let Some(ty) = self.infer_expr_type(arg) {
    ...
    fn compile_var_decl(&mut self, decl: &bolide_parser::VarDecl) -> Result<(), String> {
        ...
        if is_main && self.scope_depth == 0 && self.global_refs.contains_key(&decl.name) {
            if let Some(ref value) = decl.value {
                ...
                let raw_val = match value {
                    Expr::List(items)
                        if matches!(target_ty.as_ref(), Some(BolideType::List(_))) =>
                    {
                        self.compile_list_with_hint(items, target_ty.as_ref())?
                    }
                    Expr::Dict(entries)
                        if matches!(target_ty.as_ref(), Some(BolideType::Dict(_, _))) =>
                    {
                        self.compile_dict_with_hint(entries, target_ty.as_ref())?
                    }
        ...
        // 借用来源检查：在借用存活期间不允许对来源重新声明/释放旧绑定
        ...
        match &assign.target {
            Expr::Ident(var_name) => {
                ...
                if self.value_var_fields.contains_key(var_name) {
                    let field_vals = self.compile_value_expr(&assign.value)?;
                ...
                if is_global {
                    let gv = self.global_refs[&resolved_global];
                    let addr = self.builder.ins().global_value(self.ptr_type, gv);
                    let global_ty = self.global_var_types.get(&resolved_global).cloned();

                    if let Some(BolideType::Custom(vt_name)) = global_ty.clone() {
                        if let Some(fields) = self.value_types.get(&vt_name).cloned() {
                            let field_vals = self.compile_value_expr(&assign.value)?;
                            for (fi, _) in fields.iter().enumerate() {
                                self.builder.ins().store(
                    ...
                    // 先编译新值
                    let raw_val = match &assign.value {
                        Expr::List(items)
                            if matches!(global_ty.as_ref(), Some(BolideType::List(_))) =>
                        {
                        ...
                        _ => self.compile_expr(&assign.value)?,
                    };
                ...
                // 旧值必须在 RHS 编译完成后再释放；否则 `x = x + ...`
                // 会先释放 x，再从 RHS 读取已释放的 x。
                ...
                let raw_val = match &assign.value {
                    ...
                    Expr::Dict(entries)
                        if matches!(var_ty.as_ref(), Some(BolideType::Dict(_, _))) =>
                    {
    ...
    fn compile_index_assign(
        ...
        match base_type {
            ...
            Some(BolideType::Dict(ref key_ty, ref value_ty)) => {
                ...
                if **key_ty == BolideType::Dynamic {
                    let actual_k_ty = self.normalize_bolide_type(
                        &self.infer_expr_type(index).unwrap_or(BolideType::Dynamic),
                    );
    ...
    fn compile_return(&mut self, expr: Option<&Expr>) -> Result<(), String> {
        ...
        if let Some(e) = expr {
            ...
            let return_ty = self
                ...
                .cloned()
                .flatten();
            let mut val = if let (Some(actual_ty), Some(return_ty)) = (&val_ty, &return_ty) {
                let actual_ty = self.normalize_bolide_type(actual_ty);
                self.prepare_value_for_storage(raw_val, &actual_ty, return_ty)?
            } else {
                raw_val
            };
            // FuncSig 返回值统一 wrap 为闭包对象
    ...
    fn compile_for(&mut self, for_stmt: &bolide_parser::ForStmt) -> Result<(), String> {
        ...
        if let Some(BolideType::Custom(name)) = self.infer_expr_type(&for_stmt.iter) {
            let is_gen = crate::generators::is_generator_class_name(&name);
            let has_next = self
                .find_method_with_arg_count(&name, "next", Some(0))
                .is_some();
            if is_gen || has_next {
                if vars.len() != 1 {
                    return Err(
    ...
    fn compile_list_for(&mut self, for_stmt: &bolide_parser::ForStmt) -> Result<(), String> {
        ...
        if owns_temp_iter {
            let iter_scope_idx = self.enter_scope();
            self.remove_temp_rc_value(iter_val);
            let iter_name = format!("__for_iter_hold_{}", self.var_counter);
            self.define_variable(&iter_name, iter_val, iter_ty.clone())?;
            let iter_var = *self
                .variables
                .get(&iter_name)
                .ok_or("internal error: temporary loop iterator not materialized")?;
            let loop_iter = self.builder.use_var(iter_var);
            self.compile_list_iteration_loop(&for_stmt.vars, loop_iter, elem_type, &for_stmt.body)?;
            self.leave_scope(iter_scope_idx)?;
            return Ok(());
        }
        self.compile_list_iteration_loop(&for_stmt.vars, iter_val, elem_type, &for_stmt.body)?;
    ...
    fn compile_list_iteration_loop(
        ...
        self.builder.def_var(loop_var, init);
        ...
        let elem = if matches!(
            ...
            self.emit_list_get_inline(list_ptr, idx, &elem_type)?
